...
def get_redshift(event_id):
    ...
    # Lone SN cases
        ...
        # Table with triggertimes
        z = conn.execute("SELECT z FROM SQLDataGRBSNe WHERE SNe=?", (sn_name,))
...
def elapsed_time(dataframe, trigtime):
    ...
    for i in range(len(dataframe["date"])):
        ...
        # Alert me that I have encountered a date format that isn't supported yet.
            ...
            raise Exception(
                "No date2time function found for "
                + dataframe["date_unit"][i]
                + " , time to write a new \
...
def nondetections(df, wave_range):
    ...
    if df2.empty is False:
        ...
        df.iloc[index_list3, df.columns.get_loc(i)] = df.iloc[
            index_list3, df.columns.get_loc(j)
        ]
    ...
    for file in file_list:
        ...
        if any(substring in file.lower() for substring in radio_filetags):
            ...
            data["reference"] = len(data["time"]) * [
                file_sources.at[
                    file_sources[file_sources["Filename"] == file].index[0], "Reference"
                ]
            ]
        ...
        # Optical master db
            ...
            if "mag_type" in keys:
                if data["mag_type"][0] != "absolute":
                    ...
                    data["reference"] = len(data["time"]) * [
                        file_sources.at[
                            file_sources[file_sources["Filename"] == file].index[0],
                            "Reference",
                        ]
                    ]
        ...
        # Spectra files
            ...
            if "reference" not in data.columns.tolist():
    ...
    if "readme.txt" in file_list:
        for file in file_list:
            ...
            if any(substring in file.lower() for substring in radio_filetags):
                ...
                if "flux_density_limit" not in list(
                    data.keys()
