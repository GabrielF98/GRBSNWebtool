...
def remove_columns_and_save(file_path, columns_to_remove):
    try:
        ...
        df = pd.read_csv(file_path, delimiter='\t')  # Assuming the text file has tab-separated values (adjust delimiter if needed)
        ...
        df.to_csv(temp_file_path, sep='\t', index=False)  # Adjust the delimiter as per your requirement
